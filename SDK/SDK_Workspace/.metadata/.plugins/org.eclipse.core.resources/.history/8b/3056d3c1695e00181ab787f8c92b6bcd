...
int main()
{
	...
	while(1){
		...
		int mapNum=(int)(buf[0]);
		set_cursor(220);
		print_char(XPAR_VGA_PERIPH_MEM_0_S_AXI_MEM0_BASEADDR,mapNum+48);
		char* map=all_maps[mapNum];

		//print_char(XPAR_VGA_PERIPH_MEM_0_S_AXI_MEM0_BASEADDR,mapNum+48);
		//char* map=all_maps[mapNum];
		print_matrix(1540,map);
		set_cursor(373);
		//print_char(XPAR_VGA_PERIPH_MEM_0_S_AXI_MEM0_BASEADDR,'M');
		print_string(XPAR_VGA_PERIPH_MEM_0_S_AXI_MEM0_BASEADDR, string_s, 16);
		set_cursor(392);
		print_string(XPAR_VGA_PERIPH_MEM_0_S_AXI_MEM0_BASEADDR,buf,sizeof(buf));
